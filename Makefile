...
shell:
	@echo 'To activate the venv use source ~/.venv/task_app/bin/activate . Use deactivate to exit'

lint:
	# starts a pipenv shell, shows autopep8 diff and then fixes the files
	# does the same for isort
	@echo '---Running autopep8---'
	${bin_path}/autopep8 task_app -r -d
	${bin_path}/autopep8 task_app -r -i
	@echo '---Running isort---'
	${bin_path}/isort task_app --diff
	${bin_path}/isort task_app --atomic
